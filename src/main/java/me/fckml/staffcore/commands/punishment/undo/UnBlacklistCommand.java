...
public class UnBlacklistCommand extends BaseCommand {
    ...
    public UnBlacklistCommand() {
        super("unblacklist", "core.command.unblacklist");
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length < 2 && sender instanceof ConsoleCommandSender) {
            sender.sendMessage(CC.translate("&cUsage: /unblacklist <player> <reason> [-s]"));
            return;
        }
        ...
        if (punishment == null) {
            if (sender instanceof ConsoleCommandSender) {
                sender.sendMessage(CC.translate("&cThat player has no blacklists yet."));
                return;
            }
